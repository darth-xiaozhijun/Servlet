...
public class ServletContextServlet extends HttpServlet {
	...
	protected void service(HttpServletRequest req, HttpServletResponse response) throws ServletException, IOException {
		...
		System.out.println(servletContext==servletContext3);
		//使用ServletContext对象完成数据共享
		//数据存储
		servletContext.setAttribute("str", "ServletContext对象学习");
		//获取项目web.xml的全局配置数据
		...
		String path=servletContext.getRealPath("/doc/login.txt");
		System.out.println(path);
		//获取项目根目录下资源的流对象
		InputStream is = servletContext.getResourceAsStream("/doc/login.txt");
	}
